...
[TST](https://github.com/lytang63/temporal-state-transport) 到 MiniMax H3 的实验性移植。原论文只在 Wan2.2 上验证，未验证 H3；本节点是工程适配，不是经过验证的配置。

**具体用途。** TST 在推理时改善 H3 视频的时间稳定性——不训练、不用 LoRA、不加载额外模型。它主要针对的问题：帧间闪烁或变形的小细节（logo、画面文字、纹理、粒子）、人物和服装的身份漂移、以及不符合物理的运动（物体在帧间滑动、融化而不是真正运动）。建议作为常开补丁使用，从 `tau=0.2` 开始，对快速运动、可读文字、长视频收益最明显；用 `tau=0` 跑基线做对比。它不能凭空生成模型里没有的细节，也不能替代更好的提示词、更多的步数或更强的 checkpoint——它是对注意力状态的小幅定向校正，不是画质倍增器。

TST 不盲目加强时间注意力，而是先做诊断：碎片化传输（注意力质量集中在过少帧上）会让细节漂移，过度混合（质量过于均匀地摊开）会破坏运动物理。
