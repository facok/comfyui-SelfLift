...
- **SelfLift Progressive Sampler (Image)** (`selflift`): the paper's SelfLift-zero for compatible rectified-flow image backbones. Use it to speed up few-step image generation: most denoiser evaluations run at reduced spatial resolution and only the rest at full resolution, at the price of one VAE decode → upscale → re-encode round trip at the transition. Paper-validated settings: Z-Image-Turbo (8 steps: `transition_step=6`, `rho=0.3`) and FLUX.2-Klein (4 steps: `transition_step=3`, `rho=0.4`). Connect 4D image latents (e.g. *Empty Latent Image*), the standard Euler sampler, the model's normal scheduler, and the model's own VAE for the pixel anchor. Other rectified-flow image backbones should work but are unvalidated; results are best when the backbone's few-step output already has stable global structure, since the low-resolution prefix decides the composition.
...
An experimental port of [TST](https://github.com/lytang63/temporal-state-transport) to MiniMax H3. The paper evaluates Wan2.2, not H3; this node is an engineering adaptation, not a validated configuration.

**What it is for.** TST improves the temporal stability of H3 generations at inference time — no training, no LoRA, no extra model. Typical problems it counteracts: fine details that flicker or morph between frames (logos, on-screen text, textures, particles), identity drift of people and outfits, and physically implausible motion (objects sliding or melting between frames instead of moving). Use it as an always-on patch at `tau=0.2`, especially for fast motion, readable text, and longer videos; run `tau=0` for a baseline comparison. It cannot create detail that is not in the model, and it is not a substitute for a better prompt, more steps, or a stronger checkpoint — think of it as a small, targeted correction of the attention state, not a quality multiplier.

TST diagnoses temporal attention instead of blindly strengthening it: fragmented transport (attention mass concentrated on too few frames) drifts details, while over-mixing (mass spread too uniformly) breaks motion physics.
